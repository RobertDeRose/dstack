...
def test_failed_native_parse_restores_previous_formula(
    ...
    assert not (workspace / subject.PRIME_FILENAME).exists()


def test_formula_update_refuses_to_replace_custom_prime(git_repo: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    workspace = git_repo / ".beads"
    workspace.mkdir()
    prime = workspace / subject.PRIME_FILENAME
    prime.write_text("user-owned prime\n", encoding="utf-8")
    ...
    with pytest.raises(DstackError, match="PRIME.md"):
        subject._install_formula(subject.FormulaContext(git_repo, workspace, "bd version 1.2.2 (test)"), update=False)
    assert prime.read_text(encoding="utf-8") == "user-owned prime\n"


def test_formula_update_invokes_only_native_formula_verification(
    git_repo: Path, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    workspace = git_repo / ".beads"
    workspace.mkdir()
    ...
    assert result["prime_changed"] is True
    assert (workspace / subject.PRIME_FILENAME).read_bytes() == subject.prime_path().read_bytes()
