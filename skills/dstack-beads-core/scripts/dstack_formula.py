...
def formula_path(name: str) -> Path:
    ...
    return package_root() / "formulas" / f"{name}.formula.toml"
...
def ensure_infrastructure(root_arg: Path) -> dict[str, Any]:
    """Initialize Beads and expose the current packaged formula contracts."""

    root, initialized = ensure_beads_initialized(root_arg)
