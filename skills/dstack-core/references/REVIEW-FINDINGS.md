...
review bead stores finding records with the `Finding:` prefix. The last record for a `finding_id` is its canonical
current disposition; earlier records remain audit history.
...
- `finding_id`: immutable within the review bead and reused when its disposition changes;
...
in the historical audit section, but do not present them as unresolved findings or recommendations.
