...
Use `/implement-task <task-selector>` for exactly one open standalone `task`, `bug`, `chore`, `spike`, or `feature`. It claims only the selected issue, loads bounded context, validates, runs one fresh reviewer, commits evidence, and closes that issue. It does not create feature design or close-out records. A feature epic or child of a `workflow:feature` epic must use `/start-feature` or `/implement-feature` instead.
