...
- `dstack init` uses idempotent Beads initialization without generic hooks or agent setup, rejects unhealthy or
  malformed workspace discovery, and installs the packaged feature formula and `.beads/PRIME.md`. New feature work
  requires the installed formula to match committed `HEAD` policy.
...
- Implementation tasks use concise one-line `Implementation:` notes as canonical commit material. Each fragment names
  one concrete delivered change and is no more than 96 characters. Task titles default to `feat(feature-slug)` and may
  ...
  and replays descendants without folding unrelated fixups into it.
...
- `dstack check feature` validates all relevant feature evidence while bounding summary output. Commit paths are read
  and checked internally when ownership or documentation boundaries require them; there is no separate path-detail
  option.
