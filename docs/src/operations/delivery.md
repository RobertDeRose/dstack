...
A project may add an ordinary delivery task or native `gh:pr` gate when delivery must be represented in Beads. That task
belongs to the project's formula or reviewed implementation graph; dStack does not infer a hidden delivery phase from an
open root and does not maintain PR-gate replacement state.
