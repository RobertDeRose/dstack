...
The universal tool set is hk `1.49.0`, Node `lts`, and the `latest` Cocogitto, Harper CLI, mdBook, uv, rumdl, typos, and
`npm:markdown-table-formatter` releases. Both hk Pkl imports use `1.49.0`.
...
The universal tool count remains nine; `gh` is an external administrative prerequisite, not a mise tool. Pages requires
