...
Specification review treats current behavior as an expected implementation gap when the design explicitly defines the
replacement, owning task, acceptance, migration/cutover, compatibility, and failure behavior. Readiness receives a
digest-verified transient Beads graph projection with lifecycle and implementation tasks, edges, ownership, validation,
and commit boundaries, so embedded Dolt never requires reviewer filesystem access. Structured review records are
schema-validated and appended without shell interpolation. A roadmap-only legacy epic uses `promote-legacy-feature.py`,
the canonical digest-bound, idempotent promotion path that preserves its root identity while adding formula-defined
lifecycle and planned implementation children plus complete metadata.
