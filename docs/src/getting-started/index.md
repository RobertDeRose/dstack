...
By default, those resources are installed under `~/.pi/agent`. If Pi uses another directory, set `PI_CODING_AGENT_DIR`
or pass `--agent-dir` to `dstack install`.
...
Review turns the plan into bounded implementation tasks and presents the resulting scope, risks, and material decisions.
Read that result and explicitly approve the proposed scope before implementation begins.
