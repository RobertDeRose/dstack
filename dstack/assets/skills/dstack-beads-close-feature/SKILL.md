...
1. Close the implementation workstream when all required children are complete:
   ...
   ```bash
   dstack ctl feature finish-workstream [feature]
   dstack ctl feature claim-closeout [feature]
   dstack ctl feature scaffold-reconciliation [feature]
   ```
...
3. Run the repository's full/release validation and review the complete candidate diff after any fixups or rebase. If a
   ...
   or delivery. Correct material findings, rerun affected checks, and commit only real code/docs changes with the
...
4. Validate the current mdBook and run the documentation policy guard against the target and candidate, then:
   ...
   dstack ctl feature finish-closeout [feature]
   ```

## Delivery

- `ready`: run `dstack ctl delivery inspect <feature>` and stop with the root open.
