...
1. Inspect and claim native ready work:
   ...
   ```bash
   dstack ctl feature claim-next [feature] [--task <id>]
   ```
...
6. Stage only the intended task boundary and commit through:
   ...
   ```bash
   dstack ctl git commit --bead <task-id> --subject "<subject>"
   ```

7. Verify the committed footer and changed paths before completion:

   ```bash
   dstack ctl evidence commits --bead <task-id> --ref <base>..HEAD
   dstack ctl feature finish-task <feature> --task <task-id>
   ```
